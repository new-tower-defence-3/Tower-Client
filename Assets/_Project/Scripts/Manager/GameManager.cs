...
public class GameManager : MonoSingleton<GameManager>
{
    ...
    public int GetRandomTowerPositionIndex(int max = 0)
    {
        ...
        {
            // 0부터 roads1.Count - 1
            rand = Util.Random(0, roads1.Count);
            count++;
            if (count > roads1.Count) break;
        } while (usesTowerPositions.Contains(rand));

        // rand가 유효한지 확인
        if (rand < 0 || rand >= roads1.Count)
        {
            Debug.LogError($"유효하지 않은 타워 위치 인덱스: {rand}, roads1.Count: {roads1.Count}");
            return 0;
        }
    ...
    IEnumerator GameLoop()
    {
        ...
        {
            var rand = GetRandomTowerPositionIndex();
            var tower = AddTower(roads1[rand].transform.position + new Vector3(2*Util.Random(100, 500), -2 * Util.Random(50, 100), 0));
            tower.Init(DataManager.instance.GetData<TowerDataSO>("TOW00001"));
            usesTowerPositions.Add(rand);
        }
        while (isGameStart)
